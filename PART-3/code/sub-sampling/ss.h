...
double queuedensity(Mat &im1,Mat &image2, vector<Point2f>& source, vector<Point2f>& desti, vector<Point2f>& rec) {
...
void incre(vector<double> &r,int x, Mat &emptyframe,VideoCapture cap,vector<Point2f>& source, vector<Point2f>& desti, vector<Point2f>& rec) {
	//ofstream req;
	//vector
	int ct1 = 1;
    ...
    {   
		...
		if (ct1 % x == 1) {
			...
			if (ct1 > 1) {
				...
				double x1 = queuedensity(frame2, emptyframe, source, desti, rec);
				r.push_back(x1);
